abstract class OAuth2::AccessToken
    def self.new(pull : JSON::PullParser)
        ...
        token_type ||= "bearer"
        raw_scope = scope.nil? ? nil : scope.join(" ")
        
        case token_type.downcase
        when "bearer"
            Bearer.new(access_token, expires_in, refresh_token, raw_scope, extra)
        when "mac"
            Mac.new(access_token, expires_in, mac_algorithm.not_nil!, mac_key.not_nil!, refresh_token, raw_scope, Time.utc.to_unix, extra)
        else
